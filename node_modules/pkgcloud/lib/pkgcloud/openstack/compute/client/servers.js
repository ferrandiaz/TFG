...
exports.createServer = function createServer(details, callback) {
  ...
  if(details.hypervisor){
    createOptions.body.server.availability_zone = details.hypervisor;
  }
...
exports.migrateServer = function migrateServer(server,hypervisor,callback){
  var self = this,
  ...
  var options={
    method :'POST',
    path: urlJoin(_urlPrefix,serverId+'/action'),
    body:{"os-migrateLive":{"host": hypervisor,"block_migration":"true","disk_over_commit":"false"}}
  }
  return this._request(options,function(err,body){
  //  console.log(options);
    if(err) callback(err);
    if(!body) return new Error('Unexpected empty response');
    else callback(body);
  })
